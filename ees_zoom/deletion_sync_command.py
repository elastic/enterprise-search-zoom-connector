...
from .constant import (BATCH_SIZE, CHANNELS, RECORDINGS,
...
class DeletionSyncCommand(BaseCommand):
    ...
    def collect_multithreaded_objects_ids(
        ...
        try:
            ...
            for time_dependent_object in self.config.get_value("objects"):
                if time_dependent_object in [RECORDINGS]:
    ...
    def refresh_storage(self, deleted_ids_list):
        ...
        # recordings objects older than last month can't be fetched from the Zoom API
        ...
        for document in storage_with_collection["delete_keys"]:
            if document["type"] in [RECORDINGS] and is_within_time_range(
